...
function calibrateRotation(refRotation, otherRotation) {
  ...
  return function(newRotMatrix) {
    const newQuat = prepMatrix(newRotMatrix);
    
    return convertRealData(newQuat, deltaQuat);
  };
}
...
function convertRealData(newQuat, deltaQuat) {
  ...
  return Quaternion.multiply(newQuat, deltaQuat);
}
...
function prepMatrix(mat) {
  return Quaternion.createFromRotationMatrix(Quaternion.expandMatrix(mat));
}
