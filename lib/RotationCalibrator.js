...
function convertRealComponent(newRotationComponent, deltaComponent) {
  ...
  var rawAngle = Math.acos(newRotationMatrix[i]);
  // The difference
  var delta = differenceBetweenComponents(rawAngle, deltaMatrix[i]);
  // Convert the angle
  // Then redo the cosine
  var finalVal = Math.cos(rawAngle - delta);
...
function rotationDeltaMatrix(refMatrix, otherMatrix) {
  var outputMatrix = [];
  
  for (var i = 0; i < referenceMatrix.length; i++) {
    var delta = differenceBetweenComponents(refMatrix[i], otherMatrix[i]);
