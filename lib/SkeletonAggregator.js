...
function reconstructSkeleton(skeletons) {
	...
	switch (skeletons.length) {
		case 0:
			return {};
		case 1:
			console.log('single skel');
  		// Only one sensor was rigged up anyway
  		return skeletons[0];
  	default:
  		console.log('multi skel', skeletons);
...
function reduceJoints(previousValue, currentValue, index, array) {
  ...
  if (returner.positionConfidence < currentValue.positionConfidence) {
    ...
    returner.position[0] = currentValue.position[0];
    returner.position[1] = currentValue.position[1];
    returner.position[2] = currentValue.position[2];
  }
