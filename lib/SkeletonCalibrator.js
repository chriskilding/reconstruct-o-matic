...
function calibrate(refSkeleton, otherSkeleton) {
  const delta = calibrateSkeleton(refSkeleton, otherSkeleton);
  
  return function(receivedData) {
    // TODO implement the real version of this
...
function calibrateSkeleton(refSkeleton, otherSkeleton) {
  const combined = _.zip(_.values(refSkeleton), _.values(otherSkeleton));    
  
  const preOut = _.chain(combinedJointDataArray)
...
function averageRotation(combinedJointDataArray) {
  return _.chain(combinedJointDataArray)
    // Just operate on rotation deltas
    .pluck('rotationDeltaQuaternion')
    // Add up all the rotation deltas
    // starting with a blank quaternion
    .reduce(function(memo, posDeltaVec) {
      return ???
    }, Quaternion.create())
    // This yields a single 'sum' vector
    ...
    // And there's your average rotation!
    .value();
}
