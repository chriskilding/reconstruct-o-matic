...
function calibrate(refSkeleton, otherSkeleton) {
  ...
  const calibrated = jointRecurse(Object.keys(refSkeleton), Object.keys(otherSkeleton));

  // and then average them in some way
  const averaged = 
...
function jointRecurse(refJoints, otherJoints) {
  return refJoints.map(function(refJoint, index) {
    return calibrateJoint(refJoint, otherJoints[index]);
  });  
}
