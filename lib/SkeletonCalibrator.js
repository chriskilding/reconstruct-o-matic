...
function computeJointDelta(refJoint, otherJoint) {
    ...
    var rotatedPosition = Vector3.applyQuaternion(Vector3.vectorAsObject(otherJoint.position), deltaQuat);
...
function calibrateJoint(refJoint, otherJoint) {
    var isRefZero = _.isEqual(refJoint.rotation, zeroes);
    var isOtherZero = _.isEqual(otherJoint.rotation, zeroes);
    
    console.log('isRefZero', isRefZero, 'isOtherZero', isOtherZero);
...
function calibrate(refSkeleton, otherSkeleton) {
    ...
    return function (realSkeleton) {
        ...
        if (delta) {
            return reconstructSkeleton(_.values(realSkeleton), delta.positionDelta, delta.rotationDelta);
        } else {
            return realSkeleton;
        }
    };
